...
<div id="header" style="background-image: url(image/upper.jpg); background-repeat: no-repeat; background-size: cover;"><h1><a href="index.php">Appointment Booking Portal</a></h1></div>
  <div id="wrapper">
	...
	if(isset($_POST["submit"]))
		{
			$sql="SELECT * FROM patient WHERE username='{$_POST["username"]}' and Name='{$_POST['name']}'";
			$res=$db->query($sql);
			if($res->num_rows>0)
			{
				$s="update patient set age='{$_POST["age"]}',address='{$_POST["address"]}',mobile='{$_POST["mobile"]}',mail='{$_POST["email"]}' WHERE Name='{$_POST['name']}' and username='{$_POST["username"]}'";
				$db->query($s);
